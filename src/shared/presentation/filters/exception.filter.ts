...
import { FlattenValidationErrors, ValidationException } from './validation.exception';
import { get } from 'lodash';
import { HttpStatusMessages } from '../constants/http-status-messages.constant';
import { ErrorCodes } from '@shared/domain/exceptions/error-codes.constant';
	...
	private handleHttpException(exception: HttpException): HandleHttpExceptionResponseInterface {
		...
		if (Array.isArray(exceptionResponse.message)) {
			exceptionResponse.message.forEach((message) => {
				...
				(errors as unknown as Record<string, string[]>)[fieldName] = [message];
			});
		} else {
